...
def match_score(client_norm: str, siren_norm: str, 
                         client_city: str = "", siren_city: str = "") -> float:
    # Nettoyage des noms
    cclean = str(client_norm)
    sclean = str(siren_norm)
    # Similarité floue (token sort ratio)
    score = fuzz.token_sort_ratio(cclean, sclean)
    # Bonus pour sous-chaîne seulement si les deux noms sont assez longs
    if len(cclean) > 4 and len(sclean) > 4:
        if cclean in sclean or sclean in cclean:
            score += 10  # bonus modéré
    # Bonus/penalité géographique
    if client_city and siren_city and client_city.strip().lower() == siren_city.strip().lower():
        score += 10
    else:
        score -= 10
    return score
